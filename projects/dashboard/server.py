...
STATUS_JSON_FILE = REPO_ROOT / "status.json"
...
def read_status_json() -> dict[str, Any]:
    """Read the structured status.json written by auto-loop.sh after each cycle."""
    try:
        return json.loads(STATUS_JSON_FILE.read_text(encoding="utf-8"))
...
def gather_status_payload(system_name: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "consensusHead": read_text_file(CONSENSUS_FILE, "(no consensus file)")[:12000],
        "logTail": read_tail(LOG_FILE, lines=180),
        "cycleStatus": status_json,
    }
